...
*
\******************************************************************************/
template <class T> FCHashIntItem<T>::FCHashIntItem( int key, T *value, bool deleteItem)
{
	_key = key;
	_value = value;
	...
	if(IsRefControlObject && !std::is_base_of<FCRefControl, T>::value)
		throw FCException(0, NULL, "FCHashIntItem::FCHashIntItem", 0, FC_INT_ERR, "Class should be derived from FCRefControl to be used with this template class", __FILE__, __LINE__);
#endif
#endif
...
/*^****************************************************************************\
*
*   Destructor: FCHashIntItem
*
\******************************************************************************/
template <class T> FCHashIntItem<T>::~FCHashIntItem()
{
...
*
\******************************************************************************/
template <class T> void FCHashIntItem<T>::IncRef()
{
...
*
\******************************************************************************/
template <class T> void FCHashIntItem<T>::DecRef()
{
	if (_value && IsRefControlObject)
...
*
\******************************************************************************/
template <class T> RelationType FCHashIntItem<T>::ComparedTo( FCHashIntItem<T> *otherItem ) const
{
	if( _key < otherItem->GetItemKey() )
		return LESS;
	else if( _key > otherItem->GetItemKey() )
		return GREATER;
	else 
		return EQUAL;
}
